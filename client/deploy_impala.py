...
def generate_run_scripts_params(args, host, job_name, instance_id):
  ...
  task_id = deploy_utils.get_real_instance_id(instance_id)
  script_dict = {
    ...
    "params":
      "-webserver_port=%d " % (service_config.get_base_port(job.base_port, task_id) + 1) +
      "-be_port=%d " % (service_config.get_base_port(impalad.base_port, task_id) + 2) +
      "-planservice_port=%d " % (service_config.get_base_port(impalad.base_port, task_id) + 3) +
      "-state_store_port=%d " % (service_config.get_base_port(statestored.base_port, task_id)) +
      "-state_store_subscriber_port=%d " % (service_config.get_base_port(statestored.base_port, task_id) + 1) +
  ...
  if job_name == "impalad":
    script_dict["params"] += "-beeswax_port=%d " % (service_config.get_base_port(impalad.base_port, task_id))
    script_dict["params"] += "-hs2_port=%d " % (service_config.get_base_port(impalad.base_port, task_id) + 4)
