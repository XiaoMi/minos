...
from metrics.rpcinterface import SystemMetricsRPCInterface
...
class StatusView(MeldView):
    ...
    def make_callback(self, namespec, action):
        ...
        metrics = ('metrics', SystemMetricsRPCInterface(supervisord))

        rpcinterface = RootRPCInterface([main, system, metrics])
...
class MetricsView(MeldView):
  def make_callback(self, action, format):
    message = None
    supervisord = self.context.supervisord
    ...
    # deferred world, so just use it
    metrics = ('metrics', SystemMetricsRPCInterface(supervisord))
    rpcinterface = RootRPCInterface([metrics])

    if action == 'get_all_metrics':
      callback = rpcinterface.metrics.get_all_metrics
      def get_all_metrics():
        result = callback(format)
        if not result:
          return NOT_DONE_YET
        return result
      get_all_metrics.delay = 0.05
      return get_all_metrics

    elif action == 'get_cpu_metrics':
      callback = rpcinterface.metrics.get_cpu_metrics
      def get_cpu_metrics():
        result = callback(format)
        if not result:
          return NOT_DONE_YET
        return result
      get_cpu_metrics.delay = 0.05
      return get_cpu_metrics

    elif action == 'get_load_avg_metrics':
      callback = rpcinterface.metrics.get_load_avg_metrics
      def get_load_avg_metrics():
        result = callback(format)
        if not result:
          return NOT_DONE_YET
        return result
      get_load_avg_metrics.delay = 0.05
      return get_load_avg_metrics

    elif action == 'get_memory_metrics':
      callback = rpcinterface.metrics.get_memory_metrics
      def get_memory_metrics():
        result = callback(format)
        if not result:
          return NOT_DONE_YET
        return result
      get_memory_metrics.delay = 0.05
      return get_memory_metrics

    elif action == 'get_disk_metrics':
      callback = rpcinterface.metrics.get_disk_metrics
      def get_disk_metrics():
        result = callback(format)
        if not result:
          return NOT_DONE_YET
        return result
      get_disk_metrics.delay = 0.05
      return get_disk_metrics

    elif action == 'get_network_metrics':
      callback = rpcinterface.metrics.get_network_metrics
      def get_network_metrics():
        result = callback(format)
        if not result:
          return NOT_DONE_YET
        return result
      get_network_metrics.delay = 0.05
  ...
  def render(self):
    ...
    self.callback = self.make_callback(action, format)
    metrics = self.callback()

    root = self.clone()
    metricsbody = root.findmeld('metricsbody')
    metricsbody.content(metrics)
    return root.write_xhtmlstring()
